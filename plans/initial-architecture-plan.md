...
8. Field documentation metadata generates `doc:"${comment}"` tag if not empty.
...
- Pipeline:
  ...
  3. apply custom per-column metadata such as `doc` tags during generation
  4. post-process generated model structs with Go AST rewrite when needed for remaining tag/type rewrites
- Prefer AST rewrite over regex.

Documentation metadata plan:

- SQLite schema reflection is not the source of truth for field docs.
- Migration files should stay aligned with a generator-owned documentation mapping at column granularity.
- The generator should expose a hook shaped like `func applyDocTag(table metadata.Table, column metadata.Column) (tag string)`.
- `applyDocTag` should return a full struct tag fragment such as `doc:"Business meaning here"` when documentation exists.
- The mapping should be maintained alongside migrations so schema and generated docs do not drift.
- Missing documentation for newly added business columns should be treated as a generator validation failure.

Generator pipeline validation:

- Walens should assume a wrapper-owned generation pipeline, not direct customization inside Jet generated output.
- The practical flow is: migrate a local SQLite schema, generate Jet models/query code, then apply deterministic rewrites for field types and tags.
- Type rewrites should be scoped to generated model structs and helper code that directly depends on those model field types.
- Validation for naming-based mappings should happen before or during rewrite so unexpected schema names fail early instead of generating partially-correct models.
- The generator wrapper should be the single place that encodes these mapping rules.
...
- timestamp rewrites should use an explicit helper such as `isUnixMilliTimestampColumn(...)` instead of blindly rewriting every integer field ending in `_at`
...
- external/source-owned identifiers must never use `_id` naming, otherwise they will be treated as internal UUID references by codegen
...
- Walens should not depend on SQLite comment reflection for `doc` tag generation.
- `doc` tags should come from generator-managed column metadata kept in sync with migrations.
...
- since native column comments are unavailable for this workflow, Walens should treat generator-managed column documentation metadata as the source of truth for field documentation

### WALENS-4 / P0.4 - Go-Jet customization pipeline

Validated on 2026-04-06 against the current architecture requirements for UUID IDs, Unix-millisecond timestamps, duration wrappers, doc tags, identifier naming, and Huma schema integration.

Outcome:

- the desired Go-Jet customization path is feasible, but it should be owned by a Walens wrapper pipeline rather than by editing generated files manually
- UUID field mapping is feasible and should remain naming-driven for `id` and `*_id`
- Unix-millisecond timestamp mapping is feasible, but should use an explicit field-name matcher instead of broad integer-column heuristics
- duration wrapper mapping is feasible for `*_ms` fields as long as Walens reserves that suffix for true duration columns
- `doc` tag generation is feasible via generator-managed metadata and should stay outside SQLite reflection
- `*_identifier` naming remains necessary to keep external IDs from being misclassified as UUID relations
- Huma `SchemaProvider` integration is feasible through shared custom wrapper types used by API-facing models

Recommended implementation shape:

- run migrations into a temporary/local SQLite schema used only as the codegen source database
- run Jet generation normally against that schema
- run a deterministic Walens post-generation step that rewrites generated model field types and struct tags
- centralize mapping predicates in helpers such as `isUUIDColumn`, `isUnixMilliTimestampColumn`, `isDurationColumn`, and `applyDocTag`
- keep custom wrappers implementing `database/sql` conversion interfaces so rewritten generated models still scan and write correctly
- keep Huma `SchemaProvider` on the shared wrapper types that appear in transport models

Why this shape was chosen:

- it avoids hand-editing generated files
- it keeps the mapping logic explicit and reviewable in one place
- it matches the existing need for generator-managed documentation metadata
- it reduces the risk of accidental type rewrites caused by overly-broad schema heuristics

Watch-outs:

- broad suffix-only timestamp rules can incorrectly rewrite non-timestamp integer fields
- broad `_id` rules will misclassify external IDs if naming discipline slips
- wrappers used in generated DB models must implement `driver.Valuer` and `sql.Scanner` correctly or Jet queries will break at runtime
- Huma schema behavior should stay attached to transport-facing wrapper types, not inferred from raw generated DB models alone
...
Validated on 2026-04-06 against the no-CGO deployment target and the planned Go-Jet customization path.
...
- keep SQL schema migrations authoritative for database structure
- keep field documentation metadata authoritative in the codegen wrapper at column granularity
- use a helper shaped like `func applyDocTag(table metadata.Table, column metadata.Column) (tag string)` to emit `doc:"..."`
- require migration changes and documentation metadata changes to be updated together when columns are added or changed
- fail generation when required field documentation metadata is missing for business tables

Why this shape was chosen:

- it preserves the SQLite and no-CGO deployment goals
- it avoids depending on unsupported SQLite comment reflection behavior
- it keeps generated model documentation deterministic and explicit per field
- it is a reasonable trade-off because Walens already needs a codegen wrapper for other Jet customizations

Watch-outs:

- migration files and documentation metadata can drift if updated separately
- column renames must update both schema and documentation mapping in the same change
- generator validation should fail loudly so missing docs do not silently disappear from models
- migration 1 should stay SQLite-setup-only even if documentation metadata for later business schema exists elsewhere
...
- migrations and generator-owned documentation metadata can drift if they are not updated together.

Mitigation options:

- generator-managed per-column documentation mapping consumed by `applyDocTag`
...
- keep field documentation metadata in the generator layer and update it with schema changes
- do not rely on SQLite reflection alone to recover documentation later
